...
const getVeoModelName = () => {
  return localStorage.getItem('veoModel') || 'veo-3.0-fast-generate-001';
};
...
const genImageClick = async (editor: Editor) => {
  ...
  try {
    const numberOfImages = parseInt(
      localStorage.getItem('numberOfCreations') || '1',
      10,
    );
    imageObjects = await generateImages(promptText, image, numberOfImages);
  } catch (e) {
    editor.select(placeholderIds[0]);
    editor.deleteShapes(placeholderIds);
...
const genVideoClick = async (editor: Editor) => {
  ...
  let videoObjects = [];
  try {
    const numberOfVideos = parseInt(
      localStorage.getItem('numberOfCreations') || '1',
      10,
    );
    videoObjects = await generateVideo(image, promptText, numberOfVideos);
  } catch (e) {
    editor.select(placeholderIds[0]);
    editor.deleteShapes(placeholderIds);
    throw new Error(e.message);
...
const OverlayComponent = track(() => {
  ...
  const handleGenerateStoryboard = (prompts: string[]) => {
    if (!prompts || prompts.length === 0) return;

    // Position the storyboard relative to the current viewport
    const viewport = editor.getViewportPageBounds();
    const startX = viewport.minX + 50;
    const startY = viewport.minY + 50;
    ...
    prompts.forEach((prompt, index) => {
      ...
      editor.createShape({
        id: textShapeId,
        type: 'text',
        x: startX + index * (VIDEO_WIDTH / 2 + 100),
        y: startY,
        props: {
          richText: toRichText(prompt),
          autoSize: false,
  ...
  return (
    <div
      ...
      {showSettings && (
        <Settings onClose={() => setShowSettings(false)} addToast={addToast} />
      )}
      {showStoryboardGenerator && (
        <StoryboardGenerator
          ...
          addToast={addToast}
          onSubmit={(prompts) => {
            handleGenerateStoryboard(prompts);
      ...
      <PromptBar
        ...
        onOpenStoryboardGenerator={() => setShowStoryboardGenerator(true)}
